...
(function init(param) {
  ...
  var classNames = classAreasDictionary.keys()
    ...
    .filter(ee.Filter.stringContains('item', 'OBJECTID_1').not());

  
 print('here',classNames)
...
function getMosaic(paths, regionId, variables, gridName) {
  ...
  // if(country === 'BOLIVIA' || country === 'ECUADOR' || country === 'VENEZUELA'|| country === 'PERU') {
    
    joinedMosaics = ee.ImageCollection(paths)
      .filterMetadata('region_code', 'equals', Number(mosaicRegion))
      .map(function(image) {
        return image.addBands([altitude, slope, Hand_bands, slppost, shademask2]);
      });
  ...
  //         return ee.Image.cat(primary, secondary, altitude, slope, Hand_bands, slppost, shade_mask2);//,shade_mask2
  //       })
  //     );
  //   }
  // }
