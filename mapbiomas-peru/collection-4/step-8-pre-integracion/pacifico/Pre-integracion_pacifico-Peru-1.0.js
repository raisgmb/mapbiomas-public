...
var prefixo_out = 'Integracion-'+ param.pais+ '-' + param.version_output;
// print(integracion_v1)
...
for (var yearI = 0; yearI < param.years.length; yearI++) {
  ...
  var clasificacionYear = ClassGeneralColl.filter(ee.Filter.eq('year', param.years[yearI])).min()
  Map.addLayer(clasificacionYear,
      vis, 'classifGeneral' + '-' + param.years[yearI]
    );

  var yearManglar = Manglar5.filter(ee.Filter.eq('year', param.years[yearI])).min()
  var yearMosaictransv9 = mosaictransv9.filter(ee.Filter.eq('year', param.years[yearI])).min()
  var yearFnnfinund = fnnfinund11.filter(ee.Filter.eq('year', param.years[yearI])).min()
  var yearMosaictransv18 = mosaictransv18.filter(ee.Filter.eq('year', param.years[yearI])).min();
  var yearUrbano = urbano24.filter(ee.Filter.eq('year', param.years[yearI])).min();
  var yearMining = mining30.filter(ee.Filter.eq('year', param.years[yearI])).min();
  var yearGlaciar = Glaciar34.filter(ee.Filter.eq('year', param.years[yearI])).min(); 
  ...
  listTrans.forEach(function(transv){
    Map.addLayer(transv[0], vis, transv[1] + '-' + param.years[yearI],false
  );
  })

  Map.addLayer(
    integracion_v1,
    vis2, 'integracion' + '-' + param.years[yearI]
  );
