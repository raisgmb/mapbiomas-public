...
// INTEGRACION PACIFICO COLECCION 1.0 COSTA
// GRUPO DE DESARROLLO 
...
var mosaictransv21 = ee.ImageCollection('projects/mapbiomas-raisg/TRANSVERSALES/' + param.pais + '/COLECCION4/AGRICULTURA/INTEGRACION/agricultura4')
  .filterMetadata('country', 'equals', param.pais)
  .filterMetadata('version', 'equals', param.versionTransversal.mosaictransv18.toString())
  .filter(ee.Filter.stringContains('system:index', 'PACIFIC'))
  .map(function(image){return image.updateMask(image.eq(21))})
  .map(function(image){return image.rename('classification')})
  // .mosaic()
  // .eq(18).multiply(18)
  // .selfMask();
...
years.forEach(function(year){
  ...
  var assetsClasificaciones = {
        ...
        '21-T': mosaictransv21.filter(ee.Filter.eq('year', year)).min(),
...
for (var yearI = 0; yearI < param.yearsVis.length; yearI++) {
  ...
  var yearMosaictransv21 = mosaictransv21.filter(ee.Filter.eq('year', param.yearsVis[yearI])).min();
