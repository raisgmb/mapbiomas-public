 ...
 *  
 */  
var outputAsset = 'projects/mapbiomas-raisg/COLECCION4/INTEGRACION/integracion-region';
...
var App = {
    ...
    options: {
        ...
        assets: { 
            ...
            regions: 'projects/mapbiomas-raisg/DATOS_AUXILIARES/ESTADISTICAS/COLECCION4/country-raster',
            ...
            wet: 'projects/mapbiomas-raisg/TRANSVERSALES/PERU/COLECCION4/INUNDABLE/INTEGRACION/inundable114',
        ...
        var prevalenceList = [
              ...
              'exception': {
                    'rule': [
                        {
                            'class_input': [
                                App.options.ids.per_reg70208,
                            ],
                            'source': App.options.assets.regionsRaster
                        },
                        {
                            'class_input': [21],
                            'source': App.options.assets.agr + App.options.prefix.per + App.options.year + '-' + App.options.versions.agr
              ...
              
              {
                ...
                'rule': {
                    'class_input': 11,
                    'class_output': 11,
                    'source': App.options.assets.wet + App.options.prefix.per + App.options.year + '-' + App.options.versions.wet
                },
              'exception': null,
              'exception2': {
                    'rule': [
    ...
    loadLayers: function () {
        App.options.layers = [
            ...
            {
                'name': 'wetland',
                'data': ee.Image(App.options.assets.wet + App.options.prefix.per + App.options.year + '-' + App.options.versions.wet).selfMask(),
                'shown': false
            },
            {
                'name': 'Mining',
...
var regMosaicRaster = ee.Image(regMosaic);
regMosaicRaster = regMosaicRaster.eq(701).or(regMosaicRaster.eq(702)).selfMask();

var grid = grids.filter(ee.Filter.eq('feature_id', 13));

Export.image.toAsset({
   'image': integrated.updateMask(regMosaicRaster).selfMask(),
   'description': 'PERU' + '-' + outputVersion + '-AMAZ',
   'assetId': outputAsset + '/' + 'PERU' + '-' + outputVersion + '-AMAZ',
   'pyramidingPolicy': {
       ".default": "mode"
   },
