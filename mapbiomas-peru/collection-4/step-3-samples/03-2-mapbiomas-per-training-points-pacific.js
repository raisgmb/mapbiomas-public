...
var param = {
  ...
  ciclo: 'ciclo-1',
  region_RAISG : false, // si es una region RAISG poner: true , pero si es Pacifico: False
};
...
(function init(param) {
  ...
  if(param.region_RAISG){
     regionMask   = ee.Image(assets.RAISG.regionsRaster_RAISG).eq(regionId).selfMask()
     regionVector = ee.FeatureCollection(assets.RAISG.regions_RAISG)
                    .filterMetadata("id_regionC", "equals", regionId);  
  }
  ...
  var classNames = classAreasDictionary.keys()
    ...
    .filter(ee.Filter.neq('item', 'OBJECTID_1')); //add

   print('classAreasDictionary',classAreasDictionary)
...
function getMosaic(paths, regionId, variables, gridName,regionVector) {
 ...
 // var joinedMosaics = mosaics[0];
  ...
  //   var joinedCollection = join.apply(mosaics[0], mosaics[1], joiner);
    ...
    //     return ee.Image.cat(primary, secondary, altitude, slope,longLat,Hand_bands,slppost);//,shade_mask2
    //   })
    // );
  // }
