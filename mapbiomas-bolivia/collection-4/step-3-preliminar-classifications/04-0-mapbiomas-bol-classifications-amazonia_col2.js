...
var altitude = ee.Image("JAXA/ALOS/AW3D30_V1_1"),
    ...
    region_rasig = ee.FeatureCollection("projects/mapbiomas-raisg/DATOS_AUXILIARES/VECTORES/clasificacion-regiones-3"),
    geometry_11 = 
    /* color: #d63000 */
    /* shown: false */
...
var region = ee.FeatureCollection('projects/mapbiomas-raisg/DATOS_AUXILIARES/VECTORES/clasificacion-regiones-4-buffer-250m')
...
years.evaluate(function(years, error){
  ...
  years.forEach(function(year) {
    ...
    var yearTrainingSamples = ee.FeatureCollection(
      ee.Algorithms.If(
        contained,
        ee.FeatureCollection(
          trainingSamples + '-' + year + '-' + 'p032-' + samplesVersion),
        null
      )
    );
  ...
  var filename = country + '-' + regionId + '-' + outputVersion+'-c2';
